...
<script lang="ts">
  ...
  async function redraw(): Promise<void> {
    if (!canvasEl || !rawPageCanvas) return;
    const token = renderToken;
    if (mode === "source") {
      paintSource(rawPageCanvas);
      return;
    }
    ...
    paintFinal(bitCanvas);
  }

  // 元PDFモード: PDF.js が描画したページをそのまま、528:792 の枠に収まるよう
  // 縮小表示するだけ（回転・クロップ・二値化などは適用しない。§7.7）。
  function paintSource(source: HTMLCanvasElement): void {
    if (!canvasEl) return;
    canvasEl.width = OUTPUT_WIDTH;
    canvasEl.height = OUTPUT_HEIGHT;
  ...
  function paintFinal(source: HTMLCanvasElement): void {
    if (!canvasEl) return;
...
<div class="pv-wrap">
  <div class="pv-page">
    <canvas bind:this={canvasEl} width={OUTPUT_WIDTH} height={OUTPUT_HEIGHT}></canvas>
  </div>
  <div class="pv-pager">
...
<style>
  .pv-wrap { display: flex; flex-direction: column; align-items: center; gap: 12px; }
  .pv-page {
    width: 100%; max-width: 220px; aspect-ratio: 528 / 792; background: #fff;
