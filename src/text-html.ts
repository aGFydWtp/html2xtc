...
`
  ...
  /* 縦書きは html（ルート）に指定する。入れ子要素に writing-mode を付けると
     ...
     orphans/widows: Chrome の UA 初期値は 2 で、段落の先頭/末尾に1行だけ
     残すページ分割を禁止するため、段落が丸ごと次ページへ送られ、その分だけ
     1列（fontSizePx × lineHeight）丸ごと空白のページが生じる。機構自体は
     src/epub/html.ts の縦書き分岐（buildFinalCss の writingModeRule）と同じ
     ― !important が不要な理由・ルート配置の理由など一般論はそちらのコメント
     を参照し、ここでは重複させない。

     この修正は2件（orphans/widows と、下の .content p のマージン全辺
     リセット）セット。前者だけでは段落間隔の設定（paragraphSpacingEm）が
     UAのマージン相殺で無視されたままになり、後者だけでは（orphans/widows
     がUA既定の2のままなので）1列欠落そのものは解消されない ― 両方揃って
     初めて意味を持つ。

     実測（夏目漱石『こころ』由来の plain テキスト、1,331段落/162,902字、
     fontSizePx:24 line-height:1.8 → 1列=43.2px、X3 528x792、margin 0、
     CSS注入なしの生成コード実測、1列以上欠落ページの割合。分母は完全な
     白紙ページを除いた実効ページ数 ― このサンプルには白紙が1ページ含まれる
     ため総ページ数と1ずれる。src/epub/html.ts の姉妹コメントは分母＝総
     ページ数だが、あちらのサンプルは白紙0ページで単にズレなかっただけで、
     計測規約自体は同じ）:
       修正前（main、本番CSSのまま）                195/539 (36.2%)、総540ページ
       修正後（既定 paragraphSpacingEm: 0.9）        49/526  (9.3%)、総527ページ（−2.41%）
       修正後 + paragraphSpacingEm: 0                 0/462  (0.0%)、総463ページ
     ベースライン（段落マージンにUAの1emが残ったまま）はバンドが3系統に
     分裂し境界が曖昧なため、この行の割合には±1〜2ppの不確かさがある。

     既定の paragraphSpacingEm:0.9 でもゼロにはならない。ヒストグラムは
     20px(211)/42px(266)/63〜65px(49)の三峰: 42px=20+21.6（段落間隔1個分の
     端数で、これは正常 ― 欠落ではない）、63px=20+43.2（1列欠落、上表の
     49件はここ）。残る9.3%はorphans/widowsの取りこぼしではなく、段落境界
     で終わるページで残り約53pxに対し「段落マージン21.6 + 次の1行43.2 =
     64.8」が入らないケース ― Chromeは改ページ位置で段落の末尾マージンを
     切り捨てないため、段落間隔を持つこと自体が生む幾何的コストであり、
     CSS側にこれ以上の修正は要らない。paragraphSpacingEm:0で厳密に0.0%に
     なることがこの説明を裏付ける。

     paragraphSpacingEm が実際にページ数へ反映されるようになった証拠
     （同一入力、CSS注入なしの生成コード実測）:
                              修正前      修正後
       paragraphSpacingEm 0.9   528ページ    527ページ
       paragraphSpacingEm 0     528ページ    463ページ
       判定                 完全一致(無効)  64ページ差(有効)
     段落境界のオンセット間隔の最頻値も67/68px→64/65pxに移動した
     （64.8=43.2+21.6(0.9em)。修正前の67.2=43.2+24(UAの1em)ではなくなった）。

     修正後の paragraphSpacingEm:0・463ページのラスタは、CSS注入（
     margin:0 + orphans/widows:1 の手動トグル）版の463ページとラスタ単位で
     全ページバイト一致 ― 生成コード経路と注入検証経路が同一出力であること
     の実証。比較はラスタ化したページ同士であり、PDFファイル自体のバイト
     比較ではない（PDFは描画ごとのメタデータを持つため）。

     青空文庫入力でも改善している（「ほぼ効かない」ではない）: 同じ
     『こころ』を aozora レンダラに通すと 45/480(9.4%)・総481ページ →
     8/477(1.7%)・総478ページ（−0.62%）。残り8件のうち3件は
     ［＃改ページ］×2と底本コロフォンのbreak-before:page直前ページ（正当な
     短ページ）で、実質の残存は5/477(1.0%)。効いたのはorphans/widowsでは
     なく(2)のマージン全辺リセットの側 ― aozoraレンダラは連続行を1つの<p>
     内で<br>で繋ぐため<p>224個に対し<br>1120個となり、481ページに対し
     ブロック境界が約0.5個/ページしかなく、orphans/widowsはブロックの
     先頭/末尾の行にしか効かないためこちらは今もほぼ効く余地がない、という
     説明自体は正しいまま ― それでも改善したのは、そのわずかな<p>境界
     ひとつひとつでもマージン相殺の解消が効いたため。空行区切りのソースを
     渡せば<p>1331個・<br>0個になりplainと同じ挙動になる（生成のみ確認）。

     横書き分岐には適用しない: 横組みでは orphans/widows:2 が正しい既定で
     あり、横書き分岐は既に margin: 0 0 var(--paragraph-spacing) で全辺
     リセット済みで健全なため、下のマージン相殺も起きない。

     トレードオフ: orphans/widows:1 は段落の先頭/末尾1行がページ端に孤立
     する「泣き別れ」を許容する。日本語縦組みでは慣行的に許容される代償
     で、ページ丸ごと1列分の空白を防ぐ方を優先する。 */
  html {
  ...
  .content p {
    /* UA既定の p { margin-block-start: 1em; margin-block-end: 1em } の
       margin-block-start が残ったまま margin-block-end だけを指定すると、
       隣接する段落間でブロック方向マージンが相殺(collapse)しUA側の1emが
       勝つ ― paragraphSpacingEm が1em以下の値のとき完全に無視される。
       実測（修正前）: paragraphSpacingEm 0 と 0.9 で改ページが完全に同一
       （どちらも528ページ、ヒストグラム一致）。段落境界のオンセット間隔の
       実測は67/68px = 43.2(1行) + 24(1em)で、0.9emなら64.8pxのはずが65px
       はほぼ観測されない。横書き分岐（margin: 0 0 var(--paragraph-spacing)）
       と同じ意図になるよう、先に margin: 0 で全辺をリセットしてから
       margin-block-end だけを上書きする。効果・残存する幾何的コスト（既定
       paragraphSpacingEm:0.9では9.3%残存、paragraphSpacingEm:0でのみ0.0%）
       は上の html ルートのコメントを参照。 */
    margin: 0;
