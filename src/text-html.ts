...
`
  ...
  /* 縦書きは html（ルート）に指定する。入れ子要素に writing-mode を付けると
     ...
     リセット）セットで、片方だけでは1列欠落ページが0%にならない。実測
     （夏目漱石『こころ』由来の plain テキスト、1,331段落/162,902字、
     fontSizePx:24 line-height:1.8 → 1列=43.2px、X3 528x792、margin 0、
     トグル注入による対照実験、1列以上欠落ページの割合）:
       ベースライン（本番CSSのまま）        195/539 (36.2%)、総540ページ
       orphans/widows:1 のみ                 59/527 (11.2%)、総528ページ（−2.22%）
       段落マージン除去のみ                  140/474 (29.5%)、総475ページ
       両方（今回の修正）                    0/462 (0.0%)、総463ページ（−2.53%）
     ベースラインは左端空白が20px(324ページ)と63px(123ページ)の二峰ヒストグ
     ラム、修正後は20px単峰(最大22px)。63−20=43=ちょうど1ピッチ分。本番CSS
     （段落マージンあり）ではバンドが3系統に分裂し49px帯の境界が曖昧なため、
     上記割合には±1〜2ppの不確かさがある。

     青空文庫入力ではほぼ動かない: 同じ『こころ』を aozora レンダラに通すと
     45/480(9.4%) → 41/480(8.5%)、総ページ481 → 481（±0%）。理由はテキスト
     形式ではなくHTML構造 ― aozoraレンダラは連続行を1つの<p>内で<br>で繋ぐ
     ため<p>224個に対し<br>1120個となり、481ページに対しブロック境界が
     約0.5個/ページしかない。orphans/widowsはブロックの先頭/末尾の行にしか
     効かないため効く余地がない。空行区切りのソースを渡せば<p>1331個・
     <br>0個になりplainと同じ挙動になる（生成のみ確認）。
  ...
  .content p {
    /* UA既定の p { margin-block-start: 1em; margin-block-end: 1em } の
       ...
       実測: paragraphSpacingEm 0 と 0.9 で改ページが完全に同一（どちら
       も528ページ、ヒストグラム一致）。段落境界のオンセット間隔の実測は
       67/68px = 43.2(1行) + 24(1em)で、0.9emなら64.8pxのはずが65pxは
       ほぼ観測されない。横書き分岐（margin: 0 0 var(--paragraph-spacing)）
       と同じ意図になるよう、先に margin: 0 で全辺をリセットしてから
       margin-block-end だけを上書きする。単独では効果は限定的（29.5%
       残存、上のコメント参照）― orphans/widows:1 とセットで初めて1列
       丸ごと空白のページが0%になる。 */
    margin: 0;
